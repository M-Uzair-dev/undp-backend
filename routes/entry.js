...
router.get('/entry/:serialNumber', async (req, res) => {
  try {
    ...
    const daysSinceCommissioning = calculateDaysSince(entry.dateOfCommissioning);
    ...
    const response = {
      ...
      calculatedValues: {
        ...
        woodSaved: `${woodSavedKg.toFixed(2)} kg`,
...
router.get('/statistics', async (req, res) => {
  try {
    ...
    const statistics = {
      ...
      averageDaysSinceCommissioning: entries.length > 0 ? (totalDays / entries.length).toFixed(1) : 0,
      totalWoodSaved: `${totalWoodSaved.toFixed(2)} kg`,
